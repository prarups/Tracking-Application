  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {roles.map((r) => (
          <div key={r.id} className="glass-card p-6 rounded-3xl shadow-md space-y-4 relative overflow-hidden border border-slate-200 dark:border-slate-800">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-bold text-slate-900 dark:text-white text-base">{r.name}</h3>
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{r.description || 'Custom RBAC Role'}</p>
              </div>
              <span className="bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-500/30 font-bold px-2.5 py-1 rounded-full text-[10px] shadow-sm">
                {r.user_count} Users
              </span>
            </div>
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80">
              ...
              <div className="flex flex-wrap gap-1.5">
                {r.permissions.map((p) => (
                  <span key={p} className="bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 text-[10px] font-mono font-medium px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm">
                    {p}
                  </span>
                ))}
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/70 backdrop-blur-md p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-xl p-6 max-h-[90vh] overflow-y-auto text-slate-900 dark:text-white">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3 mb-4">
              <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Shield className="w-5 h-5 text-purple-600 dark:text-purple-400" /> Create Dynamic Custom Role
              </h3>
            ...
            <form onSubmit={handleCreateRole} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Role Title <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  required
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  className="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-purple-500 focus:outline-none shadow-sm"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-200 dark:border-slate-800">
                    ...
                    return (
                      <div
                        ...
                        className={`p-2.5 rounded-xl border cursor-pointer transition-all flex items-center justify-between text-xs ${
                          isChecked
                            ? 'bg-purple-100 dark:bg-purple-950/40 border-purple-300 dark:border-purple-500/40 text-purple-900 dark:text-white font-bold shadow-sm'
                      ...
                      >
                        ...
                        {isChecked && <CheckSquare className="w-4 h-4 text-purple-600 dark:text-purple-400" />}
                      </div>
                    );
                  })}
                </div>
