    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs">
            ...
            >
              <option value="" className="bg-slate-900 text-slate-100">All Department Groups</option>
              {groups.map((g) => (
                <option key={g.id} value={String(g.id)} className="bg-slate-900 text-slate-100">
                  Group: {g.name} [{g.code}]
                </option>
              ))}
            </select>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white px-4 py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-blue-600/30 cursor-pointer"
          >
          ...
          <tbody className="divide-y divide-slate-800/60">
            {fields.map((f, i) => (
              <tr key={f.id} className="hover:bg-slate-800/40 transition-colors">
                <td className="px-4 py-3 font-mono font-bold text-blue-400">#{i + 1}</td>
                <td className="px-4 py-3 font-semibold text-white">{f.label}</td>
                <td className="px-4 py-3 font-mono text-slate-400">{f.field_key}</td>
                <td className="px-4 py-3 font-medium">
                  {f.group_details ? (
                    <span className="inline-flex items-center gap-1 bg-purple-500/20 text-purple-300 border border-purple-500/30 px-2 py-0.5 rounded text-[11px] font-bold">
                ...
                <td className="px-4 py-3">
                  <span className="bg-blue-600/20 text-blue-300 border border-blue-500/30 font-bold px-2 py-0.5 rounded text-[10px]">
                    {f.field_type}
                  </span>
                </td>
                <td className="px-4 py-3">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Plus className="w-4 h-4 text-blue-400" /> Create Custom Attribute
              </h3>
            ...
            <form onSubmit={handleCreateField} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">
                  Target Department Group <span className="text-red-400">*</span>
                </label>
                <select
                  required
                  value={targetGroupId}
                  onChange={(e) => setTargetGroupId(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white cursor-pointer font-semibold focus:border-blue-500 focus:outline-none"
                >
                  <option value="" disabled className="bg-slate-900 text-slate-400">Select Department Group</option>
                  {groups.map((g) => (
                    <option key={g.id} value={String(g.id)} className="bg-slate-900 text-slate-100">
                      {g.name} [{g.code}]
                    </option>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Field Key Identifier</label>
                <input
                  type="text"
                  required
                  value={fieldKey}
                  onChange={(e) => setFieldKey(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-white font-mono focus:border-blue-500 focus:outline-none"
                  placeholder="custom_sprint_name"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Field Type (20+ Supported)</label>
                <select
                  value={fieldType}
                  onChange={(e) => setFieldType(e.target.value as FieldType)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white cursor-pointer focus:border-blue-500 focus:outline-none"
                >
                  {FIELD_TYPES.map((ft) => (
                    <option key={ft.id} value={ft.id} className="bg-slate-900 text-slate-100">
                      {ft.label} [{ft.id}]
                    </option>
                  ))}
                </select>
              ...
              {(fieldType === 'DROPDOWN' || fieldType === 'SEARCHABLE_DROPDOWN' || fieldType === 'MULTI_SELECT' || fieldType === 'RADIO') && (
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Choices / Options (Comma Separated)</label>
                  <input
                    type="text"
                    value={optionsStr}
              ...
              <div className="flex items-center gap-3 pt-2">
                <input
                  ...
                  className="w-4 h-4 rounded text-blue-600 border-slate-700 focus:ring-blue-500"
                />
                <label htmlFor="is_req" className="text-xs font-semibold text-slate-300 cursor-pointer">
