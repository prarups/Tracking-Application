  ...
  return (
    <div className="flex items-center gap-2 h-full">
      <input
        ...
        className="bg-slate-950 border border-slate-700 hover:border-blue-500 rounded-lg px-2 py-1 text-xs text-slate-200 cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-500 [color-scheme:dark]"
      />
      {saving && <span className="text-[10px] text-blue-400 font-bold animate-pulse">Saving...</span>}
    </div>
  );
};
  ...
  return (
    <div className="flex items-center justify-between gap-1.5 h-full group">
      <span
        ...
        className="font-mono font-bold text-blue-400 hover:underline cursor-pointer truncate"
      ...
      <button
        ...
        className="p-1 rounded bg-slate-800/80 hover:bg-blue-600 text-slate-400 hover:text-white transition-all cursor-pointer flex-shrink-0"
      >
        {copied ? (
  ...
  const columnDefs: ColDef<Ticket>[] = [
    ...
    },
    {
      ...
      cellRenderer: (params: any) => {
        ...
        if (val === 'BACKLOG') {
          colorClass = 'bg-slate-500/20 text-slate-300 border-slate-500/40 font-bold';
        } else if (val === 'OPEN' || val === 'TODO') {
          colorClass = 'bg-sky-500/20 text-sky-300 border-sky-500/40 font-bold';
        } else if (val === 'IN_PROGRESS') {
          colorClass = 'bg-amber-500/20 text-amber-300 border-amber-500/40 font-bold';
        } else if (val === 'IN_REVIEW') {
          colorClass = 'bg-purple-500/20 text-purple-300 border-purple-500/40 font-bold';
        } else if (val === 'REOPEN') {
          colorClass = 'bg-orange-500/20 text-orange-300 border-orange-500/40 font-bold';
        } else if (val === 'DONE') {
          colorClass = 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40 font-bold';
        } else if (val === 'CLOSED') {
          colorClass = 'bg-rose-500/20 text-rose-300 border-rose-500/40 font-bold';
        }
    ...
    },
    {
      ...
      cellRenderer: (params: any) => {
        ...
        if (val === 'CRITICAL') {
          colorClass = 'bg-rose-600/30 text-rose-300 border-rose-500/50 font-black animate-pulse';
        } else if (val === 'URGENT') {
          colorClass = 'bg-orange-500/20 text-orange-300 border-orange-500/40 font-bold';
        } else if (val === 'HIGH') {
          colorClass = 'bg-amber-500/20 text-amber-300 border-amber-500/40 font-semibold';
        } else if (val === 'MEDIUM') {
    ...
    },
    {
      ...
      cellRenderer: (params: any) => {
        ...
        return <span className="text-slate-200 text-xs font-semibold">{displayName}</span>;
      },
    },
    {
    ...
    },
    {
      ...
      cellRenderer: (params: any) => (
        <span className="font-mono text-xs text-slate-300 font-medium">{params.value}</span>
      ),
    },
  ];

  return (
    <div className="ag-theme-alpine-dark w-full h-[580px] rounded-2xl overflow-hidden shadow-2xl border border-slate-800">
      {loading ? (
